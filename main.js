...
const correctAnswer = document.getElementById('correct-answer'), //кол-во правильный ответов
      numberOfAllQuestions2 = document.getElementById('number-of-all-questions-2'), //кол-во вопросов(в модальном окне)
...
const questions = [
    ...
    {
        question: 'Что называют языком разметки ?',
        options: [
            'HTML',
            'CSS',
            'Sass',
...
const randomQuestion = () => {
    ...
    let hitDuplicate = false; //якорь для поверки одинаковых вопросов
...
const checkAnswer = el => {
    if(el.target.dataset.id == questions[indexOfQuestion].rightAnswer) {
        ...
        uptdateAnswerTracker('correct');
        score++;
    } else {
        el.target.classList.add('wrong');
        uptdateAnswerTracker('wrong');
    }
    disabledOptions();
};
...
const uptdateAnswerTracker = status => {
